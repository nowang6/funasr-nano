...
class FunASRNano(nn.Module):
    ...
    def data_load_speech(
        ...
        for i, (system_prompt, user_prompt, target_out) in enumerate(
            ...
            for k, sub_str in enumerate(splits):
                if not sub_str.startswith("<|startofspeech|>"):
                    ...
                    if sub_str.startswith("!"):
                        ...
                        # 将 Fbank 特征从 [b, T, d] 调整为 [b, d, T]
                        speech = speech.permute(0, 2, 1)

                        # 计算音频编码器下采样后的长度
                        # 模拟音频编码器两次池化后的长度，确定需要多少个 fake token 来占位。
                        olens = 1 + (speech_lengths[0].item() - 3 + 2 * 1) // 2
                        olens = 1 + (olens - 3 + 2 * 1) // 2
                        fake_token_len_i = (olens - 1) // 2 + 1

                        # 在文本 token 序列中插入占位符，后续会用音频编码器的输出替换。
            ...
            # 如果存在语音数据，将其添加到 fbank 列表中，供后续编码使用。
            if len(speech) > 0:
        ...
        for batch_idx in range(batch_size):
            for turn_id in range(fbank_beg.shape[1]):
                ...
                if fbank_beg_idx > 0:
                    ...
                    speech_token = encoder_out[speech_idx,
                                               :speech_token_len, :]
        ...
        **kwargs,
        ...
        if key is None:
            ...
            for _ in data_in:
                ...
                key.append(
                    "rand_key_" + "".join(random.choice(chars)
                                          for _ in range(13))
                )
